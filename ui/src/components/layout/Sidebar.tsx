...
export const Sidebar: React.FC = () => {
    ...
    return (
        <aside className="w-64 h-full border-r bg-muted/30 flex flex-col hidden md:flex shrink-0">
            <Link to="/" className="p-4 flex items-center gap-2 border-b hover:bg-muted/50 transition-colors">
                <div className="w-6 h-6 bg-primary rounded-md flex items-center justify-center text-primary-foreground font-bold text-xs italic text-center">
                    z
                </div>
                <span className="font-semibold text-sm tracking-tight text-center">znote</span>
            </Link>

            <nav className="flex-1 overflow-y-auto p-3 space-y-4">
                <div className="space-y-1">
                    <SidebarItem
                        icon={<EntityIcon type="note" size={16} className="" />}
                        label="Notes"
                        href="/notes"
                        isActive={location.pathname === "/notes"}
                    />
                    <SidebarItem
                        icon={<EntityIcon type="bookmark" size={16} className="" />}
                        label="Bookmarks"
                        href="/bookmarks"
                        isActive={location.pathname === "/bookmarks"}
                    />
                    <SidebarItem
                        icon={<EntityIcon type="task" size={16} className="" />}
                        label="Tasks"
                        href="/tasks"
                ...
                {loading ? (
                    ...
                    <>
                        {starredNotes.length > 0 && <SidebarSection title="Notes" icon={<EntityIcon type="note" size={16} className="" />} items={starredNotes} type="note" currentPath={location.pathname} />}
                        {starredBookmarks.length > 0 && <SidebarSection title="Bookmarks" icon={<EntityIcon type="bookmark" size={16} className="" />} items={starredBookmarks} type="bookmark" currentPath={location.pathname} />}
                        {starredTasks.length > 0 && <SidebarSection title="Tasks" icon={<EntityIcon type="task" size={16} className="" />} items={starredTasks} type="task" currentPath={location.pathname} />}
                    </>
                )}
                ...
                <div>
                    <div className="px-3 mb-2 flex items-center justify-between group">
                        ...
                        <MoreVertical size={12} className="text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" />
                    </div>
                        ...
                        {tags.map(tag => {
                            ...
                            return (
                                <Link
                                    ...
                                    to={`/tag/${cleanTag}`}
                                    className={cn(
                                        "text-xs font-mono block truncate py-1 hover:underline transition-colors",
            ...
            {isOpen && (
                <div className="space-y-1">
                    ...
                    {items.length > 5 && (
                        <Link
                            to={`/${type}s`}
                            className="block px-9 py-1 text-[10px] text-muted-foreground italic hover:text-primary transition-colors"
                        >
                            + {items.length - 5} more
                        </Link>
                    )}
...
const SidebarItem: React.FC<{
    ...
    const content = (
        <div className={cn(
            "flex items-center gap-3 px-3 py-1.5 rounded-md cursor-pointer text-sm font-medium transition-colors",
